...
to illicitly gain access to another system.
...
However,
these languages solve different problems to Python.
...
Both Julia and Swift exhort their speed credentials,
but do we even need our languages to get faster? If Moore's law holds (1),
...
encrypted setting.
But this private, new world comes with a heavy computation cos
and presents a step-change in how we compute information.
If public demand for privacy comes quickly,
we may be required to adopt faster languages in the short-term before hardware catches up.

But what if we don't like any of these languages and instead want to develop our own?
...
Machine learning is slowly pervading from academia to widespread industry adoption,
so it is highly likely it will be a permanent fixture of programming in the 2030s.
...
I know,
but its popularity is still growing in several domains,
and the growing inertia in open-source libraries will only
make it more difficult
for people adopt a new language.
But whichever language does take the crown,
the management of large,
distributed data will surely be the key to its success.
...
Signal has had a huge boost in membership due to a change in WhatsApp's data sharing policy
